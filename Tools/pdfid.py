...
class cBinaryFile:
    ...
    @property
    def size(self):
        size = os.path.getsize(self.file) #9b+
        return size
...
def PDFiD(file, allNames=False, extraData=False, disarm=False, force=False):
    ...
    try:
        ...
        att = xmlDoc.createAttribute('Filesize')
        att.nodeValue = oBinaryFile.size
        xmlDoc.documentElement.setAttributeNode(att)
        if extraData:
            oPDFDate = cPDFDate()
            oEntropy = cEntropy()
...
def PDFiD2JSON(xmlDoc, force): 
    ...
    filesize = xmlDoc.documentElement.getAttribute('Filesize')
